...
class RatedDatasetCreator:
    ...
    def __init__(self):
        self.result_ds = load_dataset("SeppeV/survey_results_final", split="train")
        self.joke_ds = load_dataset(
            "SeppeV/JokeTailor_big_set_annotated", split="train"
        ).to_pandas()
    ...
    def create_rated_ds(self, ds_id):
        ...
        for index, userId in enumerate(userIds):
            ...
            jokes = [
                self.joke_ds.loc[self.joke_ds["jokeId"] == jokeId, "jokeText"].item()
                for jokeId in last_ranking
            ]
            if len(jokes) > 7:
                jokes.pop(-3)
                jokes.pop(-3)
            ...
            for i in range(7):
                ...
                ds_dict["jokeText"].extend(
                    [
                        self.joke_ds.loc[
                            self.joke_ds["jokeId"] == jokeId, "jokeText"
                        ].item()
                        for jokeId in rankings
                    ]
                )
...
rd.create_rated_ds("RatedDatasetJokeTailor")
